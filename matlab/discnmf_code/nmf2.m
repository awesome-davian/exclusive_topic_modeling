...
function [W,H,U,V,res,iter,REC]=nmf2(A,B,k,kd,alpha,beta,varargin)
	% parse parameters
	params = inputParser;
...
% 	params.addParamValue('alpha'    ,0         ,@(x) isscalar(x) & x >= 0);
% 	params.addParamValue('beta'    ,0         ,@(x) isscalar(x) & x >= 0);
...
%        W = WW; H = HH;
        W = rand(m,par.k1); H = rand(par.k1,n1); U = rand(m,par.k2); V = rand(par.k2,n2);
	else
		W = par.init.W; H = par.init.H; U = par.init.U; V = par.init.V;
	end
...
%     end
	...
	[W,H,U,V,par,val,ver] = feval(initializer,A,B,W,H,U,V,par);
...
%     figure(3);subplot(3,4,1);imagesc(W);
%     figure(4);subplot(3,4,1);imagesc(U);
    for iter=1:par.max_iter
        
		% Actual work of this iteration is executed here.
        [W,H,U,V,gradW,gradH,val] = feval(iterSolver,A,B,W,H,U,V,iter,par,val);
        if mod(iter,10)==0
%         figure(3);subplot(3,4,iter/10+1);imagesc(W);
%         figure(4);subplot(3,4,iter/10+1);imagesc(U);
        end
% % %         iter
%        [iter norm(A - W*H, 'fro')]
...
%         end
    ...
    error = norm(A-W*H,'fro')/par.n1 ;
        error00 = norm(B-U*V,'fro')/par.n2 ;
        error0 = norm(W(:,par.kd+1:par.k1)-U(:,par.kd+1:par.k2),'fro')/(par.ks);
        error1 = sum(sum(W(1:par.kd)'*U(1:par.kd)))/par.kd;% * par.beta;
%         fprintf('error: [[[[%d,, %d]]]]]],, %d,, %d total: %d %s \n',error, error00, error0, error1, error+error0+error1, 'after');
%     W'*U
...
%     final.W_density      = length(find(W>0))/(m*k1);
%     final.H_density      = length(find(H>0))/(n1*k1);
%     final.U_density      = length(find(U>0))/(m*k2);
%     final.V_density      = length(find(V>0))/(n2*k2);
% 
%     if par.verbose
% 		REC.final = final;
% 	end
...
function [W,H,U,V,par,val,ver] = comp_initializer(A,B,W,H,U,V,par)
    epsilon = 1e-16;
	for iter=1:5
        WtA = W'*A;
        WtW = W'*W;
        ...
        VVt_reg = applyReg(V*V',par,par.reg_w);
        for i = 1:par.k1
            U(:,i) = max(U(:,i) * VVt_reg(i,i) + BVt(:,i) - U * VVt_reg(:,i),epsilon);
            if sum(U(:,i))>0
                U(:,i) = U(:,i)/norm(U(:,i));
            end
        end
    ...
    [W,H]=normalize_by_W(W,H);
        ...
        for i = 0:par.ks-1
            WtU = (W(:,1:par.k1-i))'*U(:,1:par.k2-i);
            [C,I]=max(WtU); [C2,I2]=max(C); I1=I(I2);
            W(:,[par.k1-i I1]) = W(:,[I1 par.k1-i]);
            U(:,[par.k1-i I2]) = U(:,[I2 par.k1-i]);
        end
...
function [W,H,U,V,gradW,gradH,val] = comp_iterSolver(A,B,W,H,U,V,iter,par,val)
	epsilon = 1e-16;
...
%     end
	...
	HHt_reg = applyReg(H*H',par,par.reg_w);
	for i = 1:par.kd
		W(:,i) = max(W(:,i) + (AHt(:,i) - W * HHt_reg(:,i)-par.beta/2*sum(U(:,1:par.kd),2))/HHt_reg(i,i),epsilon);
		if sum(W(:,i))>0
			W(:,i) = W(:,i)/norm(W(:,i));
		end
    end
	...
	UtU_reg = applyReg(UtU,par,par.reg_h);
	for i = 1:par.k2
		V(i,:) = max(V(i,:) + (UtB(i,:) - UtU_reg(i,:) * V)/UtU_reg(i,i),epsilon);
    end    
    
    BVt = B*V';
	VVt_reg = applyReg(V*V',par,par.reg_w);
	for i = 1:par.kd
		U(:,i) = max(U(:,i) + (BVt(:,i) - U * VVt_reg(:,i)-par.beta/par.c/2*sum(W(:,1:par.kd),2))/VVt_reg(i,i),epsilon);
		if sum(U(:,i))>0
			U(:,i) = U(:,i)/norm(U(:,i));
		end
    end
    for i = par.kd+1:par.k2
		U(:,i) = max(U(:,i) * VVt_reg(i,i)/(VVt_reg(i,i)+par.alpha/par.c) + (BVt(:,i)+par.alpha/par.c*W(:,i)-U*VVt_reg(:,i)) / (VVt_reg(i,i)+par.alpha/par.c),epsilon);
		if sum(U(:,i))>0
			U(:,i) = U(:,i)/norm(U(:,i));
		end
    end    
