function [Topics, wtopk_score, topic_score, xcl_score,Freq_words] = function_run_extm_updated0316(Tdm, exclusiveness,Stop_words, voca, k, topk)
...
%     size(dict{1},1)
    ...
    %disp('Case Study using specific keyword');
    
%     search_word = 'marathon';
%     search_word_index=0;
%     S_wd_mtx = cell(2,1);
%     Sum_swd = cell(2,1);
%     for i = 1:size(dict{1},1)
%         if strcmp(search_word,dict{1}(i))
%                 search_word_index = i;
%         end
%     end
%     if search_word_index > 0
%         for i=1:2
%             S_wd_mtx{i} = NS_Mtx{i}(search_word_index,:);
%             S_wd_mtx{i} = S_wd_mtx{i} ~= 0;
%             Sum_swd{i} = sum(S_wd_mtx{i});
%     end
%     end
    %Sum_swd
    ...
    [ Wtopk,Htopk,DocTopk,Wtopk_idx,Wtopk_score,Topic_score,Wtopk_num] = parsenmf(WC,HC,dict{1},topk);
